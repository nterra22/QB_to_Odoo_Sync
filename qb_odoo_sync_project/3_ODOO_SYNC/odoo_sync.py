...
INVENTORY_XML_PATH = 'c:\\SD Code Files\\QB Odoo Sync\\QB_to_Odoo_Sync\\qb_odoo_sync_project\\2_SD_MASTER_DATABASE\\inventory.xml'
...
def sync_xml_to_odoo():
    """Syncs inventory data from the XML file to Odoo (XML → Odoo)."""
    logger.info("Starting XML to Odoo sync.")
    uid, models = connect_odoo()
    if not uid or not models:
        return
    ...
    xml_items = read_xml_inventory()
    if not xml_items:
        logger.warning("No items found in XML to sync to Odoo.")
        return

    # Fetch all Odoo products by name for quick lookup
    try:
        product_ids = models.execute_kw(ODOO_DB, uid, ODOO_API_KEY,
            'product.product', 'search', [[('type', '=', 'product')]])
        products = models.execute_kw(ODOO_DB, uid, ODOO_API_KEY,
            'product.product', 'read', [product_ids], {
                'fields': ['id', 'name', 'list_price', 'standard_price', 'description_sale', 'description_purchase', 'qty_available']
            })
        odoo_by_name = {p['name']: p for p in products}
    except Exception as e:
        logger.error(f"Error fetching Odoo products for XML sync: {e}")
        return

    for name, item in xml_items.items():
        vals = {}
        # Map XML fields to Odoo fields
        if item.findtext('SalesPrice') is not None:
            vals['list_price'] = float(item.findtext('SalesPrice') or 0)
        if item.findtext('PurchaseCost') is not None:
            vals['standard_price'] = float(item.findtext('PurchaseCost') or 0)
        if item.findtext('SalesDesc') is not None:
            vals['description_sale'] = item.findtext('SalesDesc')
        if item.findtext('PurchaseDesc') is not None:
            vals['description_purchase'] = item.findtext('PurchaseDesc')
        if item.findtext('QuantityOnHand') is not None:
            vals['qty_available'] = float(item.findtext('QuantityOnHand') or 0)
        # Add more mappings as needed

        if name in odoo_by_name:
            # Update existing Odoo product
            odoo_id = odoo_by_name[name]['id']
            try:
                models.execute_kw(ODOO_DB, uid, ODOO_API_KEY,
                    'product.product', 'write', [[odoo_id], vals])
                logger.info(f"Updated Odoo product '{name}' from XML.")
            except Exception as e:
                logger.error(f"Failed to update Odoo product '{name}': {e}")
        else:
            ...
            try:
                ...
                logger.info(f"Created new Odoo product '{name}' from XML.")
            except Exception as e:
                logger.error(f"Failed to create Odoo product '{name}': {e}")
    logger.info("XML to Odoo sync finished.")

if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == 'xml2odoo':
        sync_xml_to_odoo()
    elif len(sys.argv) > 1 and sys.argv[1] == 'odoo2xml':
        sync_odoo_to_xml()
    else:
        logger.info("Running both Odoo→XML and XML→Odoo syncs.")
        sync_odoo_to_xml()
        sync_xml_to_odoo()
